...
if use_cuda:
    X_test =  torch.from_numpy(X_test).type(torch.FloatTensor).cuda() # test features
    y_test =  torch.from_numpy(y_test).type(torch.LongTensor).cuda()# test labels
    
    X_train = torch.from_numpy(X_train).type(torch.FloatTensor).cuda()# training features
    y_train = torch.from_numpy(y_train).type(torch.LongTensor).cuda()# training labels
else: 
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features = 30,out_features = 4)
...
if use_cuda:
    ones = torch.ones(len(X_train)).unsqueeze(1).cuda()
    torch_y_numel = torch.Tensor([y_train.numel()]).cuda()
else:
    ones = torch.ones(len(X_train)).unsqueeze(1)
    torch_y_numel = torch.Tensor([y_train.numel()])

for i in range(epochs):  
    #Get Last-but-1 layer INPUT on forward pass
    # Adding column of 1s to activations (bias)
    activations = torch.cat([model.forward(X_train)[1],ones],1)
    ...
    acc = torch.div(torch.sum(torch.eq(model.predict(X_train),y_train)),torch_y_numel)
