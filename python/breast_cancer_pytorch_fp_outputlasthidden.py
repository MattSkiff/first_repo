...
X_train, X_test, y_train, y_test = sklearn.model_selection.train_test_split(X, y, test_size=0.33, random_state=rand) # splitting data randomly into test/train
...
class Net(nn.Module):
    ...
    def forward(self,x):
        ...
        #Output of the third hidden layer
        x = self.fc3(x)
        x = F.relu(x)
        
        # retrieving activations of last hidden layer
        out = x
...
def fp_loss(activations, last_but_one_classifier):
    ...
    # dot product matrix of 
    hi_xj = torch.mm(hi.t(),xj.t()) 
    
    # normising by each neuron (hi) and each instance (xj)
    norm_hi = torch.norm(hi,dim = 1)
    norm_xj = torch.norm(xj,dim = 1)
    
    # torch.div returns normalised dot products, this is then squared and the matrix sum taken
...
for i in range(epochs):
    ...
    activations = model.forward(X_train)[1]
    ...
    aug = torch.cat([layers[4],layers[5].unsqueeze(1)],1)
    
    # Calculate and add accuracy to the list
    acc = torch.div(torch.sum(torch.eq(model.predict(X_train),y_train)),torch.Tensor(np.array([y_train.numel()])))
