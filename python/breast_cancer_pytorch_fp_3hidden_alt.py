...
def fp_loss(activations, last_but_one_classifier):
    ...
    xj = activations # the output of the last-but-1 hidden layer
...
summary(model,X_train.shape,device = str(device))
...
for i in range(epochs):  
  ...
  optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad_, model.parameters()), lr=0.01)
  # for each of the loss functions
  for j in range(2):
    n_iter += 1
    if (j == 1):
        model.fc1.weight.requires_grad_ = False
        #model.fc2.weight.requires_grad_ = True
        model.fc3.bias.requires_grad_ = True
        
        model.fc1.bias.requires_grad_ = False
       #model.fc2.bias.requires_grad_ = True
        model.fc3.bias.requires_grad_ = True
    else:
    ...
    if (j == 0):
        ...
        # Print loss and acc during training (every 100 epochs)
        if i % 100 == 0:  
            print("CE Loss: ",round(loss.item(),4))
            print("Training Accuracy: ",round(acc.item(),4))
    
    #Clear the previous gradients
    optimizer.zero_grad() 
    
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad_, model.parameters()), lr=0.01)
...
# Plot of FP Losses vs Epochs
f1 = plt.figure()
plt.plot(losses_fp)
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.suptitle("Flip Probability: FP Loss vs Epochs",fontsize=14,y=0.975)
plt.title("Binary Classification of Wisconsin Breast Cancer Dataset",fontsize=10,y=1)

# Plot of CE Losses vs Epochs
f1 = plt.figure()
plt.plot(losses_ce)
plt.ylabel('Loss')
plt.xlabel('Epochs')
plt.suptitle("Flip Probability: CE Loss vs Epochs",fontsize=14,y=0.975)
plt.title("Binary Classification of Wisconsin Breast Cancer Dataset",fontsize=10,y=1)
